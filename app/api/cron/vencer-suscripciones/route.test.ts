...
function req(authHeader?: string) {
  return new Request("http://localhost/api/cron/vencer-suscripciones", {
...
describe("GET /api/cron/vencer-suscripciones", () => {
  test("CRON_SECRET no configurado -> 401", async () => {
  ...
  test("secret correcto -> 200 y llama al RPC vencer_suscripciones_y_addons", async () => {
    ...
    expect(supabase.rpc).toHaveBeenCalledWith("vencer_suscripciones_y_addons");
  });
});
